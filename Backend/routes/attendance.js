...
router.get("/history", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const present = result.rows.filter(
...
router.post("/login", verifyToken, async (req, res) => {
  ...
  const expectedLoginTime = "09:00:00"; // Assuming 9 AM is expected login time
  const isLate = loginTime > expectedLoginTime;
  ...
  try {
    ...
    await db.query(
      ...
      [employeeId, today, loginTime, status, isLate]
    );

    res.json({ message: "Login time recorded", status, loginTime, isLate });
  } catch (error) {
