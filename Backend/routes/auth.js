...
router.get("/employees", verifyToken, verifyHR, async (req, res) => {
  ...
  try {
    const result = await db.query(
      "SELECT u.id, u.email, u.role, e.id as employee_id, e.name, e.department, e.designation, e.join_date FROM users u LEFT JOIN employees e ON u.id = e.user_id ORDER BY e.name"
    );
